...
async def uplaod_data(project_id :str,file:UploadFile, app_settings: Settings =Depends(get_settings)):
    ...
    is_valid,signal=DataController().valid_uploaded_file(file=file)
    ...
    file_path=DataController().generate_unique_filename(org_filename=file.filename,project_id=project_id)
